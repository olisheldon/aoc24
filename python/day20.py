...
def min_path(grid, cheat_coord, start, end):
    ...
    min_heap = [(0, start[0], start[1])] # (steps, r, c)
    while min_heap:
        ...
        for dr, dc in NEIGHBOURS:
            if (r + dr, c + dc) == cheat_coord:
                heapq.heappush(min_heap, (steps + 2, r + 2 * dr, c + 2 * dc))
            else:
...
def part1():
    ...
    default_time = min_path(grid, (-1, -1), start, end)
    cheat_times = [min_path(grid, (r, c), start, end) for r in range(ROWS) for c in range(COLS)]
    times_in_range = list(filter(lambda x: x in range(default_time - TIME_SAVED + 1), cheat_times))
